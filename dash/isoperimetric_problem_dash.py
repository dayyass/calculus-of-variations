...
app.layout = html.Div([
    ...
    html.Div([html.Label('Enter f0'), dcc.Input(id='f0')]), html.H4(''),
    ...
    html.Div([html.Label('Enter f_list'), dcc.Input(id='f_list')]), html.H4(''),
    html.Div([html.Label('Enter alpha_list'), dcc.Input(id='alpha_list')]), html.H4(''),
...
@app.callback(
    ...
    [
        Input('f0', 'value'), Input('t0', 'value'), Input('t1', 'value'), Input('x0', 'value'), Input('x1', 'value'),
        Input('f_list', 'value'), Input('alpha_list', 'value'),
    ],
)
def update_output(f0, t0, t1, x0, x1, f_list, alpha_list):
    if (f0 is not None) & (t0 is not None) & (t1 is not None) & (x0 is not None) & (x1 is not None) & \
            (f_list is not None) & (alpha_list is not None):

        solver = IsoperimetricProblemSolver(
            f0=f0, t0=float(t0), t1=float(t1), x0=float(x0), x1=float(x1),
            f_list=f_list.split(), alpha_list=[float(alpha) for alpha in alpha_list.split()],
        )
        try:
            solver.solve()
        except:
