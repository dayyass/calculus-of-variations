...
app.layout = html.Div([
    ...
    html.Div([html.Label('Enter x1_0'), dcc.Input(id='x1_0')]), html.H4(''),
    html.Div([html.Label('Enter x1_1'), dcc.Input(id='x1_1')]), html.H4(''),
    html.Div([html.Label('Enter x2_0'), dcc.Input(id='x2_0')]), html.H4(''),
    html.Div([html.Label('Enter x2_1'), dcc.Input(id='x2_1')]), html.H4(''),
...
@app.callback(
    ...
    [
        Input('L', 'value'), Input('t0', 'value'), Input('t1', 'value'),
        Input('x1_0', 'value'), Input('x1_1', 'value'), Input('x2_0', 'value'), Input('x2_1', 'value'),
    ],
)
def update_output(L, t0, t1, x1_0, x1_1, x2_0, x2_1):
    if (L is not None) & (t0 is not None) & (t1 is not None) & \
            (x1_0 is not None) & (x1_1 is not None) & (x2_0 is not None) & (x2_1 is not None):

        solver = MultidimensionalSolver(
            L=L, t0=float(t0), t1=float(t1), x1_0=float(x1_0), x1_1=float(x1_1), x2_0=float(x2_0), x2_1=float(x2_1),
        )
        try:
            solver.solve()
        except:
